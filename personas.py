...
class Cocinero(Persona):
    ...
    def cocinar(self, informacion_plato):
        nombre, tipo, calidad, dificultad = informacion_plato

        if tipo == "Bebestible":
            tamaño = informacion_plato[4]
            plato = Bebestible(nombre, calidad, dificultad, tamaño)
            ...
            plato = Comestible(nombre, calidad, dificultad)
            self.energia -= 15

        if dificultad > self.habilidad: 
            factor_calidad = 0.7 
        else:
            factor_calidad = 1.5
    ...
    def recibir_pedido(self, pedido, demora):
        ...
        for plato in pedido:
            if plato.calidad >= 11:
                ...
                calificacion = max(0, calificacion - 3)  
